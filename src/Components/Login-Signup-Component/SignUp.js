  ...
  const loginWithGoogle = () => {
    signInWithPopup(auth, provider)
      .then((result) => {
        ...
        console.log("login allowrd with google");
        console.log(user);
        // ...
      })
      .then(() => {
  ...
  return (
    <div className="h-[100vh] bg-black " id="linear">
      ...
      <div className=" ml-[600px] mr-[600px] h-[86vh] shadow-2xl rounded-lg bg-gradient from-red-500 to-black">
        <div className="text-white font-bold text-3xl w-10/12 leading-[50px] ml-10">Sign up to start listening....</div>
        <form className="text-white mt-10" onSubmit={(e) => { e.preventDefault() }}>
          <label className="ml-12 mt-24 text-sm font-bold">Enter Full Name</label>
          <br />
          <input type="text" className="bg-black text-white border border-slate-400 rounded-lg p-2 ml-12 w-96" placeholder="Enter Full Name" ref={Name} />
          <br />
          <br />
          <label className="ml-12 text-sm font-bold">Enter Email Address</label>
          <br />
          ...
          <br />
          <br />
          <button className="border border-slate-500 bg-black font-bold w-96 p-2 ml-10 mr-10 rounded-3xl text-xl hover:scale-125 transition-all ease-in-out" onClick={handleButtonClick}>Next</button>
          <br />
          <br />
          <p className="text-red-500 font-bold text-lg ">{errorMessage}</p>
          <span className="ml-8 text-slate-500">____________________________</span><span>Or</span><span className="mr-10 text-slate-500">____________________________</span>
        </form>
        <button className="border border-slate-500 bg-black font-bold text-white p-1 w-96 ml-10 mr-10 rounded-3xl mt-6 hover:scale-125 transition-all ease-in-out" onClick={loginWithGoogle}>
          ...
          <p className="-mt-[34px] p-2">Sign Up With Google</p></button>
        <div className="flex ml-40 mt-8">
          <p className="text-slate-400 ">Already have an account?</p>
          <p className="underline text-white cursor-pointer"><Link to="/login">Log in here.</Link></p>
        </div>
      </div>
      <div className="w-1/4 text-white float-right -mt-[800px]">
        <img src={image2} alt=""></img>
      </div>
      <div className="w-1/4 text-white float-right -mt-[280px] ">
